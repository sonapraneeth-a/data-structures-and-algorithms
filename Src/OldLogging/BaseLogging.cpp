 ...
 *                                              - Moved code to BaseLogging namespace
 *                                              - Added Status of LOG -> WARNING, INFO, DEBUG, CRITICAL
 */
...
namespace BaseLogging
{
    ...
    void
    RegisterLog(unsigned int level,
                unsigned int Status,
                std::string filename)
    {
        if (!(level >= 0 && level <= 3))
    ...
    void
    PrintLogMessage(unsigned int level,
    ...
    void
    ClearLog(std::stringstream &LogMessageToPrint)
    {
